...
if (empty($errors)) {
    $check_query = "SELECT COUNT(*) FROM maintenance_summary_items WHERE maintenance_id = ? AND description = ?";
...
if (empty($errors)) {
    $insert_query = "INSERT INTO maintenance_summary_items (maintenance_id, description, quantity, unit, amount, reference) 
                     VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($insert_query);

    if ($stmt) {
        $stmt->bind_param("isidss", $maintenance_id, $description, $quantity, $unit, $amount, $reference);
