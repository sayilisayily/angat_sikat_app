...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($reference)) {
        ...
        if (move_uploaded_file($_FILES['reference']['tmp_name'], $target_file)) {
            ...
            $query = "UPDATE purchase_summary_items SET description = ?, quantity = ?, unit = ?, amount = ?, reference = ? WHERE summary_item_id = ?";
        ...
        $query = "UPDATE purchase_summary_items SET description = ?, quantity = ?, unit = ?, amount = ? WHERE summary_item_id = ?";
        $stmt = $conn->prepare($query);
